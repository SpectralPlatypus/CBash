...
bool PBEARecord::IsFlagMask(uint8_t Mask, bool Exact)
    {
    if(!XESP.IsLoaded()) return false;
    return Exact ? ((XESP->flags & Mask) == Mask) : ((XESP->flags & Mask) != 0);
    }

void PBEARecord::SetFlagMask(uint8_t Mask)
    {
    XESP.Load();
    XESP->flags = Mask;
    }

uint32_t PBEARecord::GetType()
    {
    return REV32(PBEA);
    }

char * PBEARecord::GetStrType()
    {
    return "PBEA";
    }

int32_t PBEARecord::ParseRecord(unsigned char *buffer, unsigned char *end_buffer, bool CompressedOnDisk)
    {
    uint32_t subType = 0;
    uint32_t subSize = 0;
    while(buffer < end_buffer){
        ...
        switch(subType)
            ...
            default:
                //printer("FileName = %s\n", FileName);
                printer("  PBEA: %08X - Unknown subType = %04x\n", formID, subType);
                CBASH_CHUNK_DEBUG
                printer("  Size = %i\n", subSize);
                printer("  CurPos = %08x\n\n", buffer - 6);
                buffer = end_buffer;
...
int32_t PBEARecord::WriteRecord(FileWriter &writer)
    {
    WRITE(EDID);
    WRITE(NAME);
    WRITE(XEZN);
...
bool PBEARecord::equals(Record *other)
    {
    return *this == *(PBEARecord *)other;
    }

bool PBEARecord::deep_equals(Record *master, RecordOp &read_self, RecordOp &read_master, std::unordered_set<Record *> &identical_records)
    {
