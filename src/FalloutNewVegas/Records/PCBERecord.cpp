...
namespace FNV
{
PCBERecord::PCBERecord(unsigned char *_recData):
    FNVRecord(_recData)
    {
    //
    }

PCBERecord::PCBERecord(PCBERecord *srcRecord):
    FNVRecord()
    {
    if(srcRecord == NULL)
        return;
...
PCBERecord::~PCBERecord()
    {
    //
    }

bool PCBERecord::VisitFormIDs(FormIDOp &op)
    {
    if(!IsLoaded())
        return false;
...
bool PCBERecord::IsOppositeParent()
    {
...
bool PCBERecord::IsFlagMask(uint8_t Mask, bool Exact)
    {
    if(!XESP.IsLoaded()) return false;
    return Exact ? ((XESP->flags & Mask) == Mask) : ((XESP->flags & Mask) != 0);
    }

void PCBERecord::SetFlagMask(uint8_t Mask)
    {
    XESP.Load();
    XESP->flags = Mask;
    }

uint32_t PCBERecord::GetType()
    {
    return REV32(PCBE);
    }

char * PCBERecord::GetStrType()
    {
    return "PCBE";
    }

int32_t PCBERecord::ParseRecord(unsigned char *buffer, unsigned char *end_buffer, bool CompressedOnDisk)
    {
    uint32_t subType = 0;
    uint32_t subSize = 0;
    while(buffer < end_buffer){
        ...
        switch(subType)
            ...
            default:
                //printer("FileName = %s\n", FileName);
                printer("  PCBE: %08X - Unknown subType = %04x\n", formID, subType);
                CBASH_CHUNK_DEBUG
                printer("  Size = %i\n", subSize);
                printer("  CurPos = %08x\n\n", buffer - 6);
                buffer = end_buffer;
...
int32_t PCBERecord::WriteRecord(FileWriter &writer)
    {
    WRITE(EDID);
    WRITE(NAME);
    WRITE(XEZN);
...
bool PCBERecord::operator ==(const PCBERecord &other) const
    {
...
bool PCBERecord::equals(Record *other)
    {
    return *this == *(PCBERecord *)other;
    }

bool PCBERecord::deep_equals(Record *master, RecordOp &read_self, RecordOp &read_master, std::unordered_set<Record *> &identical_records)
    {
