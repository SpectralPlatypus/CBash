...
bool PFLARecord::IsFlagMask(uint8_t Mask, bool Exact)
    {
    if(!XESP.IsLoaded()) return false;
    return Exact ? ((XESP->flags & Mask) == Mask) : ((XESP->flags & Mask) != 0);
    }

void PFLARecord::SetFlagMask(uint8_t Mask)
    {
    XESP.Load();
    XESP->flags = Mask;
    }

uint32_t PFLARecord::GetType()
    {
    return REV32(PFLA);
    }

char * PFLARecord::GetStrType()
    {
    return "PFLA";
    }

int32_t PFLARecord::ParseRecord(unsigned char *buffer, unsigned char *end_buffer, bool CompressedOnDisk)
    {
    uint32_t subType = 0;
    uint32_t subSize = 0;
...
int32_t PFLARecord::WriteRecord(FileWriter &writer)
    {
    WRITE(EDID);
    WRITE(NAME);
    WRITE(XEZN);
...
bool PFLARecord::deep_equals(Record *master, RecordOp &read_self, RecordOp &read_master, std::unordered_set<Record *> &identical_records)
    {
